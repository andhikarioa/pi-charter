...
import { ENFORCEMENT_CONSTRAINTS, type TaskContract } from '../contracts/task-contract.ts';
...
function bind(contract: ExecutionContract, snapshot: unknown): TargetBindingResult {
  return bindExecutionTarget({ execution_contract: contract, capability_snapshot: snapshot });
}
...
test('E10-A — parent: model_selection is ENFORCED', () => {
...
test('E10-B — parent: allowed_tools is INSTRUCTED, never ENFORCED', () => {
  const binding = bound(resolved(fixtureContract('critical correction')), PARENT_SNAPSHOT);
  assert.equal(binding.enforcement.allowed_tools, 'INSTRUCTED');
  assert.notEqual(binding.enforcement.allowed_tools, 'ENFORCED');
});

test('E10-C — subagents: allowed_tools is ENFORCED', () => {
  const binding = bound(subagentsReviewFixture(), SUBAGENTS_SNAPSHOT);
  assert.equal(binding.target, 'subagents');
  assert.equal(binding.enforcement.allowed_tools, 'ENFORCED');
});

test('E10-D — both canonical fixtures: allowed_files is INSTRUCTED', () => {
  const cases: [string, ExecutionContract, ExecutionTargetCapabilitySnapshot][] = [
    ['parent fixture', resolved(fixtureContract('critical correction')), PARENT_SNAPSHOT],
    ['subagents fixture', subagentsReviewFixture(), SUBAGENTS_SNAPSHOT],
  ];
  for (const [label, contract, snapshot] of cases) {
    // Both fixtures report file_scope_enforcement=false, so neither may claim a file sandbox.
    assert.equal(snapshot.capabilities.file_scope_enforcement, false, label);
    assert.equal(bound(contract, snapshot).enforcement.allowed_files, 'INSTRUCTED', label);
  }
});

test('E10-E — archaeology_off and release_forbidden are INSTRUCTED: no hard primitive is invented', () => {
  const cases: [string, ExecutionTargetCapabilitySnapshot][] = [
  ...
  for (const [label, snapshot] of cases) {
    const truth = evaluateEnforcement(snapshot.capabilities);
...
test('E10 — ENFORCED tracks exactly the capability the snapshot grants, across every combination', () => {
  // Exhaustive over all 32 capability states: the fabrication this suite exists to prevent is
  // reporting ENFORCED where the substrate has no primitive.
  for (let mask = 0; mask < 2 ** TARGET_CAPABILITY_KEYS.length; mask++) {
    ...
    const truth = evaluateEnforcement(capabilities);
...
test('E10-G — a required tool ceiling on the parent fixture fails closed', () => {
  ...
  assert.ok(error);
  assert.match(error.message, /INSTRUCTED/);
  // The missing substrate primitive is reported, not built.
  assert.match(error.message, /does not implement the missing primitive/);
});

test('E10-H — the same requirement binds on the subagents fixture with allowed_tools ENFORCED', () => {
  const binding = bound(
    carrying(subagentsReviewFixture(), { enforcement: { allowed_tools: 'required' } }),
    SUBAGENTS_SNAPSHOT,
  );
  assert.equal(binding.enforcement.allowed_tools, 'ENFORCED');
});
...
test('E10 — a required model_selection binds only where the target can select the model', () => {
  const contract = carrying(resolved(fixtureContract('critical correction')), {
    enforcement: { model_selection: 'required' },
  });
  assert.equal(bound(contract, PARENT_SNAPSHOT).enforcement.model_selection, 'ENFORCED');
  const withoutSelection = {
    ...PARENT_SNAPSHOT,
    capabilities: { ...PARENT_SNAPSHOT.capabilities, model_selection: false },
  };
  assert.deepEqual(codes(bind(contract, withoutSelection)), ['UNSUPPORTED_BY_EXECUTION_TARGET']);
});

test('E10 — archaeology_off and release_forbidden can never satisfy a hard requirement', () => {
  const cases: [ExecutionContract, ExecutionTargetCapabilitySnapshot][] = [
    [resolved(fixtureContract('critical correction')), PARENT_SNAPSHOT],
    [subagentsReviewFixture(), { name: 'subagents', capabilities: ALL_CAPABLE }],
  ];
  for (const [contract, snapshot] of cases) {
    for (const constraint of ['archaeology_off', 'release_forbidden'] as const) {
      const result = bind(carrying(contract, { enforcement: { [constraint]: 'required' } }), snapshot);
...
test('§26.1 — parent cannot satisfy an independent review requirement', () => {
  ...
  // Both missing capabilities are named in the independence refusal itself.
  assert.equal(
    errors(result).some((error) => /independent_review=false/.test(error.message)),
    true,
    JSON.stringify(errors(result)),
  );
});
...
test('§26.1 — the subagents fixture truthfully binds independent fresh-session review', () => {
  ...
  assert.equal(contract.acceptance.review?.executor, 'fresh_session');
  const binding = bound(contract, SUBAGENTS_SNAPSHOT);
  assert.equal(binding.enforcement.allowed_tools, 'ENFORCED');
  assert.equal(binding.target, 'subagents');
});
...
test('§25 — the snapshot must describe exactly the target the contract selected', () => {
  ...
  assert.deepEqual(paths(result), ['capability_snapshot.name']);
  const [error] = errors(result);
  assert.ok(error);
...
test('§25 — the binding validates the environment, not the Phase 2 artifact it is given', () => {
  ...
  if (!result.ok) return;
  // No acceptance was supplied, so no review claim is made: nothing is fabricated in either direction.
  assert.equal(result.binding.enforcement.allowed_files, 'INSTRUCTED');
...
test('§24 — unknown requirement names, values, and shapes fail closed', () => {
  ...
  const satisfied = bound(carrying(contract, { enforcement: { model_selection: 'required' } }), PARENT_SNAPSHOT);
...
test('§24 — declared truth is a closed table, never a free-form report', () => {
  const truth = bound(resolved(fixtureContract('critical correction')), PARENT_SNAPSHOT).enforcement;
  const values: EnforcementTruth[] = ENFORCEMENT_CONSTRAINTS.map((constraint) => truth[constraint]);
  assert.deepEqual(values, ['ENFORCED', 'INSTRUCTED', 'INSTRUCTED', 'INSTRUCTED', 'INSTRUCTED']);
...
test('§37 — equivalent inputs produce deep-equivalent bindings, without touching the inputs', () => {
  ...
  const snapshot = deepFreeze(structuredClone(SUBAGENTS_SNAPSHOT));
  const before = JSON.stringify({ contract, snapshot });

  const first = bound(contract, snapshot);
  const second = bound(contract, snapshot);
...
test('§37 — the resolved Phase 2 execution contract is unchanged and unextended by Phase 3', () => {
  ...
  assert.deepEqual(Object.keys(binding).sort(), ['enforcement', 'execution_contract', 'target']);
...
test('requirements.enforcement — the declaration is what the binding honours', () => {
  ...
  const truth: EnforcementTruthTable = bound(subagentsContract, SUBAGENTS_SNAPSHOT).enforcement;
  assert.equal(truth.allowed_tools, 'ENFORCED');
  assert.equal(truth.allowed_files, 'INSTRUCTED');
});
    ...
    errors: [
      {
        ...
        message:
          "execution_target=parent can only report INSTRUCTED for 'allowed_tools'; the contract requires hard enforcement and Charter does not implement the missing primitive",
      },
    ],
  });
  // Different requirements are a different contract identity with a different, honest outcome.
  const other = carrying({ ...contract }, { enforcement: { model_selection: 'required' } });
  assert.equal(bind(other, snapshot).ok, true);
});
